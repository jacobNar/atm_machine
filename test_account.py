...
def test_deposit_print_error_message_if_deposit_amount_is_negative():
    _account = account.Account()
...
def test_deposit_print_error_message_if_deposit_amount_is_zero():
    _account = account.Account()
    with pytest.raises(ValueError):
        _account.deposit(0)
...
def test_withdrawal_print_error_message_if_not_enough_funds_available():
    _account = account.Account()
...
def test_withdrawal_print_error_message_if_requested_amount_is_zero():
    _account = account.Account()
    _account.deposit(10)
    with pytest.raises(ValueError):
